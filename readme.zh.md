...
CatGo 远不止是一个结构查看器。一个窗口贯穿整条计算材料工作流——**读取几乎任何
代码的文件 → 构建与编辑 → 跑 DFT / ML / MD → 提交超算 → 分析 → 发表**——并由
AI 副驾驶（**CatBot**）和可视化工作流引擎贯穿始终。

| 模块                  | 能力                                                                                                                                                                                                                |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **3D 查看器**          | 晶体 · 分子 · 表面 · 轨迹 · 周期镜像原子 · **跨晶胞边界**渲染的键 · 可选多面体 · 按元素/按原子配色覆盖 · 正交 **与** 透视相机 · 交互式光照（ambient/key/fill/rim）· 浅/深/白/黑主题 · 实时 3D 坐标轴指示器                                                                       |
| **多面板工作区**          | 标签页 · 平铺分屏 · 弹出窗口 · 拖放面板 · 每个面板都是带独立状态的**独立**结构/轨迹 · 侧边栏结构库逐面板绑定                                                                                                                                                     |
| **打开约 30 种格式**      | VASP、Quantum ESPRESSO、CP2K、ABACUS、ORCA、Gaussian、CASTEP、SIESTA、OpenMX、LAMMPS、ASE `.traj`、phonopy、HDF5、CIF、XYZ/extXYZ、mol2、PDB、cube/CHGCAR——输入**与**输出（见[支持的文件格式](#-支持的文件格式)）                                          |
| **写出可直接运行的输入文件**    | 保格式导出到 VASP、QE、CP2K、ABACUS、ORCA、Gaussian、LAMMPS、GROMACS、AMBER、SPARK——保留你原有的 header 与关键字                                                                                                                            |
| **编辑与构建**           | 移动/添加/删除/复制粘贴原子 · 手动与自动键 · 晶格编辑（可选原子缩放）· 超胞 · 单选/多选/**框**选 · 距离/角度/二面角测量 · selective-dynamics 固定原子 · 完整撤销/重做 · 右键菜单 · 元素选择器                                                                                       |
| **结构构建器**           | Miller 指数 slab cutter · 吸附物与双吸附物放置 · moiré / 扭转双层 · 纳米管 roller（CNT/BNNT/手性）· nanoscroll · 异质结构堆叠（晶格匹配）· reticular / MOF · 掺杂与替换 · 伪氢钝化 · 水层                                                                       |
| **CatBot (AI)**     | 通过 Claude、Codex、Gemini、Ollama、DeepSeek/Qwen/Kimi/GLM 进行自然语言结构操作、构建与工作流编写——约 28 个 MCP 工具 + 按需 DFT skill 指南                                                                                                         |
| **工作流**             | 链式计算的 DAG 编辑器（opt → SP → DOS / NEB / MD / slow-growth …），含一键 Quick-Build 配方（HER、OER、ORR、NRR、CO₂RR、NEB、slow-growth、DOS）                                                                                            |
| **超算**              | SSH 终端 · 远程文件浏览 · 作业提交与监控 · SLURM / PBS / LSF / SGE · OTP + 跳板机 + SOCKS5                                                                                                                                          |
| **DFT 输入**          | 原生工作流引擎: VASP、Quantum ESPRESSO、LAMMPS、CP2K、ORCA。CatBot 草拟（skill 文本）: GPAW、ABINIT、SIESTA、DFTB+、Gaussian                                                                                                            |
| **ML / 快速计算器**      | MACE（含 mace_mp foundation）、CHGNet、M3GNet (matgl)；EMT；xTB / GFN-xTB——用于 DFT 前的廉价预筛                                                                                                                                  |
| **分析**              | DOS / PDOS、能带、COHP / ICOHP、d 带中心、布里渊区、XRD、RDF、MSD/MD 分析、电荷密度等值面、火山图、Gibbs 自由能图、相图（2D/3D/4D）                                                                                                                       |
| **催化**              | OER / HER / ORR / CO₂RR / NRR 反应路径、ICONST slow-growth 模板、C–N 偶联反应网络                                                                                                                                                |
| **导入与数据库**          | 拖放 · 粘贴 · OPTIMADE 搜索（Materials Project、MC3D、Alexandria、MaterialsCloud、OMDB、2DMatPedia）· PubChem 分子搜索                                                                                                              |
| **导出**              | 高清 PNG / JPG / TIFF / SVG / PDF 渲染 · GLB / OBJ 3D 模型 · 轨迹视频（WebM / MP4）与 PNG 序列 · DOS/XRD CSV                                                                                                                     |
| **更多**              | 文档查看器（PDF/DOCX/XLSX/CSV/Markdown/HTML）· Mol\* 生物查看器 · WebGPU 大体系渲染 · 集成终端 · 语音与手势控制 · 插件中心 · iOS / Android 构建 · English / 中文                                                                                       |

---

## 🧩 版本与适用范围

> 本页的功能与格式表描述的是 **CatGo 桌面客户端**——完整工作台。其他版本复用同一套查看器内核，只是外层的"壳"不同。

| 版本 | 能做什么 |
| --- | --- |
| **桌面客户端**（Windows · macOS · Linux） | **完整工作台**——查看、编辑、构建器、CatBot、工作流引擎、HPC 提交/监控、分析套件、全部导入/导出 |
| **网页版** — <https://app.catgo-ucsd.org> | 在浏览器中运行，使用同一套交互式查看器；依赖后端的功能（工作流*执行*、HPC）取决于是否连接到服务器 |
| **VS Code 插件** | 内嵌**完整的单窗口查看器**——在编辑器内查看、播放并**编辑**结构/轨迹，含结构工具栏（构建器、分析、OPTIMADE/PubChem 导入）、保存/导出（含远程 SSH 查看）。它**不**加载桌面壳：没有多面板标签工作区、没有独立的工作流编辑器、没有 HPC 作业管理器。详见其 [README](extensions/vscode/readme.md) |
| **Claude Code / MCP 插件**（`catbot-plugin/`） | 一个 AI 工具层（约 60 个工具），从智能体驱动**正在运行的** CatGo——它本身不渲染任何东西 |

也就是说：**仅桌面客户端（壳）独有**的部分是多面板标签工作区、可视化工作流 DAG 编辑器，以及 HPC 终端/作业管理器。编辑、构建器、导出与查看器内的工具随内嵌查看器一并提供。

---

## 📂 支持的文件格式

CatGo 通晓大多数 DFT、MD 与量子化学代码的文件语言——不止 POSCAR 和 CIF。
**读** = 打开/导入到查看器；**写** = 导出可直接运行的输入文件或结构文件
（保留 header 与关键字）。
...
| 软件 | 读 | 写 |
| --- | --- | --- |
| **VASP** | POSCAR · CONTCAR · `.vasp` · vasprun.xml · OUTCAR · XDATCAR · CHGCAR/AECCAR/LOCPOT/ELFCAR/PARCHG | POSCAR · INCAR · KPOINTS（完整输入文件集） |
| **Quantum ESPRESSO** | pw.x 输入（`.in`，scf/relax/…） | pw.x 输入 |
| **CP2K** | `.inp` · `.restart` | CP2K 输入（`&CELL`/`&COORD`/`&KPOINTS`） |
| **ABACUS** | STRU | INPUT + STRU + KPT |
| **ORCA** | `.inp` / `.out`（xyz / internal / `%coords`） | ORCA 输入 |
| **Gaussian** | `.gjf` / `.com`（+ Z-matrix）· `.log` / `.out` · cube | `.gjf` |
| **CASTEP** | `.cell` | — |
| **SIESTA** | `.fdf` | — |
| **OpenMX** | `.dat` | — |
| **LAMMPS** | `.data` / `.lmp` · dump / `.lammpstrj` | data + 运行脚本 |
| **GROMACS** | — | `.mdp` + `.gro` + `.top` |
| **AMBER** | — | mdin |
| **ASE** | `.traj`（二进制）· extXYZ | extXYZ |
| **phonopy / phono3py** | YAML | — |
| **SPARK**（kMC/MKM） | — | kMC / MKM 输入文件 |
...
| 轨迹 / MD | XDATCAR · OUTCAR · extXYZ（多帧）· ASE `.traj` · LAMMPS dump · HDF5（torch-sim / vaspout.h5）· vasprun.xml · JSON frames | extXYZ（多帧）· 视频（WebM/MP4）· PNG 序列 |
| 体数据 | Gaussian cube · VASP CHGCAR 家族 → 等值面 | — |
| 渲染与模型 | — | PNG · JPG · TIFF · SVG · PDF · **GLB** · **OBJ** |
| 文档（预览） | PDF · DOCX · XLSX/XLS/ODS · CSV/TSV · Markdown/RST · 图片 | — |

> 上面这一大批 DFT 代码格式的读取由 CatGo 的内置解析器处理。
> 少数格式仅支持导入（CASTEP、SIESTA、OpenMX、phonopy、OUTCAR、
> vasprun.xml、Gaussian 输出、cube）；其余皆可读**与**写双向往返。

---

## ⌨️ 键盘快捷键

一套密集、少用鼠标的编辑体验。下文中 `Ctrl` 表示 **Ctrl 或 ⌘**。
...
| `Ctrl`+`O` | 在活动面板中打开文件 |
| `Ctrl`+`T` | 新建结构标签页 |
| `Ctrl`+`W` | 关闭面板（若为最后一个面板则关闭标签页） |
| `Ctrl`+`Tab` / `Ctrl`+`Shift`+`Tab` | 下一个 / 上一个标签页 |
| `1` `2` `3` `4` | 激活第 *n* 个面板 |
| `Ctrl`+`Enter` | 加载粘贴的结构文本 |
| `Ctrl`+`/` | 切换 CatBot 聊天 |
...
| 快捷键 | 操作 |
| --- | --- |
| 单击 / `Ctrl`-拖拽 | 切换原子选择 / 框选（macOS 上为 ⌘-拖拽） |
| `Ctrl`+`C` / `Ctrl`+`V` | 复制 / 粘贴原子 |
| `Delete` / `Backspace` | 删除选中的原子、键或测量 |
| `Shift`+`Alt`+`Arrows` / `W` `S` | 移动选中原子（屏幕平面 / 深度） |
| `Shift`+`Arrows` / `W` `S` | 旋转选中原子（yaw / pitch / roll） |
| `Shift`-拖拽 / `Shift`+右键拖拽 | 旋转 / 翻滚选中原子 |
| `X` `Y` `Z`（按住） | 将移动/旋转锁定到单一轴 |
| `Shift` = 10× · `Ctrl` = 0.1× | 步长修饰键 |
...
| 快捷键 | 操作 |
| --- | --- |
| 左键拖拽 · 中键拖拽 · 滚轮 | 环绕 · 平移 · 缩放至光标 |
| `Arrows` / `W` `S`（未选中任何物体） | 旋转 / 翻滚相机 |
| 右键拖拽（未选中任何物体） | 翻滚相机 |
| 双击空白处 | 重置相机 |
| `r` | 将场景重新对齐到晶格 |
| `f` · `i` · `g` | 全屏 · 信息面板 · 手势模式 |
...
| `Space` | 播放 / 暂停 |
| `A` / `D` | 上一帧 / 下一帧 |
| `Ctrl`+`A` / `Ctrl`+`D` · `Home` / `End` | 第一帧 / 最后一帧 |
| `J` / `L` · `PageUp` / `PageDown` | 跳 ∓10 / ∓25 帧 |
| `0`–`9` | 跳到轨迹的 0 %…90 % |
| `+` / `−`（播放时） | 加快 / 放慢播放 |

> 应用内的完整列表位于查看器的 **信息面板 → 使用提示** 中。
